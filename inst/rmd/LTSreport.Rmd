...
if (!(is.null(l) | is.na(l))) {
  if (any(grep("png$", params$logo_file))) {
    knitr::include_graphics(path = l)
  } else {
    ...
    mtext(side = 3, line = -2.25, text = paste("Date:", format(Sys.time(), '%Y-%m-%d')), adj = 0.95)
    plot(logo_raster)
  }
}
...
The long-term stability for the reference material **`r d[,"RM"]`** with its property of **`r d[,"KW_Def"]`** was determined as \textcolor{`r ifelse(lts > 60, "green", "red")`}{`r lts`} month (`r attr(lts,"Date")`).
