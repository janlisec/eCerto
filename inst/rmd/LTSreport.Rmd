...
if (params[["dat"]]=="NA") {
  ...
  d <- data.frame("RM"="RM", "CertVal"=1L, "U_Def"="1s", "U"=1L, "KW"="KW", "KW_Def"="KW_Def", "KW_Unit"="KW_Unit", "acc_Datasets"=1, "Coef_of_Var"=1)
  v <- data.frame("Date"=as.Date(paste0("2021-",1:10,"-01")), "Value"=sort(rnorm(10)), "File"=paste("File",1:10), "Comment"=c(rep(NA,9),"Comment"))
  l <- NA
} else {
  d <- params[["dat"]][[i]][["def"]]
  v <- params[["dat"]][[i]][["val"]]
...
```{r plots, echo=FALSE, results='asis', dev.args = list(pointsize = 9), fig.height = 3.5}
  ...
  par(mfrow=c(1,2))
    plot_lts_data(x = list("def"=d, "val"=v), type=1)
    lts <- plot_lts_data(x = list("def"=d, "val"=v), type=2)
  par(mfrow=c(1,1))
```
