...
```{r setup, include=FALSE}
  ...
  #testing <- c(FALSE, TRUE)[1]
  testing <- length(params$General)==1L && is.na(params$General)
  if (testing) {
    tmp <- new.env()
    load(file = ecerto::fnc_get_local_file(x="CRM001.RData", copy_to_tempdir=FALSE), envir = tmp)
    ...
    D <- G$apm[[A]]
    L <- ecerto::fnc_get_local_file(x="BAMLogo2015.png", copy_to_tempdir=FALSE)
  } else {
    G <- params$General
...
```{r L_flt, eval=!is.null(D$lab_filter), echo=FALSE, comment=NA}
