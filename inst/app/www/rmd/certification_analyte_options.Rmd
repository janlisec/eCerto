...
The only exception is **Tab.C3**. Here, `Precision (Cert)` is used to round $\mu_c$ and $U_{abs}$ to 
...
for `Precision (Cert)` to indicate a desired rounding digit left of the decimal separator (*i.e.* $-1$ to 
...
value is displayed right over the input box of `Precision (Cert)`. It's backgroun is colored green if it is 
in accordance with the user selected value and red otherwise.
