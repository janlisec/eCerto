...
depending on the DIN/ISO regulations which apply to the certificate.

*eCerto* suggests the rounding precision for $\mu_c$ and $U_{abs}$ according to DIN 1333. The position
of the significant digit is determined based on the first non-zero digit in $U_{abs}$. The position will be
shown above the analyte parameter `Precision (cert)` upon selection of an analyte in **Tab.C3**. Once found, 
...
The user can specify negative numbers for `Precision (cert)`, *i.e.* $-1$ to round an uncertainty of 13.1 up 
to 14 or $-2$ to round 13.1 up to 20. If the uncertainty is even larger, the user should consider to upload 
data using a different (more appropriate) unit and, for instance, switch from mg/L to g/L.

***Note!***
Internally, all calculations are performed on non-rounded values (as imported). Only the visual output to
the user is diplayed rounded to improve readability.
