...
The only exception is **Tab.C3**. Here, `Precision (Cert. Val.)` is used to round $\mu_c$ and $U_{abs}$ to 
...
for `Precision (Cert. Val.)` to indicate a desired rounding digit left of the decimal separator (*i.e.* $-1$ to 
...
value is displayed right over the input box of `Precision (Cert. Val.)`. It's background is colored 
...
shown above the analyte parameter `Precision (Cert. Val.)` upon selection of an analyte in **Tab.C3**. Once found, 
