...
the user is rounded to improve readability.
...
in tables. An exception is **Tab.C3**. Here, `Precision (Cert)` is used to round $\mu_c$ and $U_{abs}$ to 
allow to comply with DIN/ISO regulations on rounding.
...
the decimal separator, *i.e.* $-1$ to round an uncertainty of 13.1 up to 20. If the uncertainty is even 
