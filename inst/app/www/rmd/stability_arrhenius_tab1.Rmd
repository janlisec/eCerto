...
the log-transformation of it's negative value `log(-k_eff)` are provided in **Tab.1**.

In the case that at least 3 finite values for $log(-k_{eff})$ exist, a linear model over 
`1/K` is calculated (depicted in **Fig.2**). Based on slope $m$ and intercept $n$ of this model values 
...
and $n=length(x_i)$ where $x_i=1/K$ together with the standard error $E$ for this model:

$$E=\sqrt{\frac{1}{n-2} \times \sum{(y_i-\overline{y})^2} - \frac{\sum{(x_i-\overline{x}) \times (y_i-\overline{y})}^2}{\sum{(x_i-\overline{x})^2}}}$$

Next, these four values $a$, $b$, $n$ and $E$ allow to compute the dependent variables:

$$u(i) = \sqrt{\frac{E^2 \times b}{(b \times n-a^2)}}$$

$$u(s) = \sqrt{\frac{E^2 \times n}{(b \times n-a^2)}}$$

$$cov = -1 \times \frac{E^2 \times a}{(b \times n-a^2)}$$
...
for an analyte in the certification module of *eCerto*, these can be used together with $CI_{upper}$ 
to calculate the storage time $S_{Month}$ for each evaluated temperature level using:

$$S_{Month}(T) = \frac{ log( \frac{\mu_c - U_{abs}}{\mu_c})}{e^{CI_{upper}(T)} }$$
