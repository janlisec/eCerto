...
-   `char` describes the *char*acteristic uncertainty, calculated as $\frac{sd}{\sqrt{n}*mean}$
...
***Note!*** `U` columns containing different uncertainty contributions are always expressed relative to the mean to allow and easy combination of the individual terms. Only `U_abs` is expressed as an absolute value.
...
-   `U_abs` specifies the absolute uncertainty and is calculated by $U_{abs}=U*cert\_val"$
