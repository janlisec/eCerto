...
# for (i in 1:2) {
# x <- as.data.frame(out[[i]])
# colnames(x) <- paste0("n=", ns)
# rownames(x) <- paste0("k=", ks)
# x <- round(x, 4)
# x <- cbind(" "= rownames(x), x)
# ft <- eCerto:::ft_default(x, caption = names(out)[i])
# flextable::bg(ft, bg = grDevices::grey(0.85), j=1)
# ft
# }
```
...
(`eCerto::cvals_Dixon`) and have been combined in a single table from <a href="http://www.statistics4u.com/fundstat_eng/cc_outlier_tests_dixon.html" target="_blank" rel="noopener noreferrer">this source</a>.
...
for 4≤n≤30 and are estimated for 31≤n≤100 based on the approximation described in <a href="https://link.springer.com/article/10.1007/s10182-011-0185-y" target="_blank" rel="noopener noreferrer">this paper</a>.
