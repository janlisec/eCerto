...
if (testing) {
  report_file <- "report_vorlage_lts.Rmd"
  ...
  tmp.env <- new.env()
  load(file = fnc_get_local_file(x="LTS001.RData"), envir = tmp.env)
  dat <- get(x = "LTS_dat", envir = tmp.env)
  d <- dat[[i]][["def"]]
  v <- dat[[i]][["val"]]
  l <- "BAMLogo2015.png"
  if (!file.exists(file.path(dirname(fnc_get_local_file(x=report_file)), params[["logo_file"]]))) {
    warning(params[["logo_file"]], " is not in same folder as ", report_file)
  }
