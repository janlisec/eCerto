...
if (knitr::is_html_output()) {
  shiny::tagList(
    shiny::tags$caption("Tab.C0 Imported analyte data from inter laboratory trial"),
    eCerto:::styleTabC0(x = df[, c("ID", "Lab", "value", "unit", "replicate", "File")], ap = D, type = "standard")
  )
} else {
  # type <- ifelse(length(unique(df[,"replicate"]))>6, "standard", "compact")
  # x <- eCerto:::get_input_data(rv = rv, type = type, excl_file = FALSE)
  # eCerto:::styleTabC0(x = x, ap = D, type = type, output = "ft")
  eCerto:::styleTabC0(x = df[, c("ID", "Lab", "value", "unit", "replicate", "File")], ap = D, type = "standard", output = "ft")
}
...
if (knitr::is_html_output()) {
  shiny::tagList(
    shiny::tags$caption("Tab.C1 Statistics regarding lab means, lab variances and outlier detection"),
...
if (knitr::is_html_output()) {
  shiny::tagList(
    shiny::tags$caption("Tab.C2 Statistics regarding overall mean distribution and variance testing"),
