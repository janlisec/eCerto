...
TestPlot <- function(data=NULL) {
  ...
  graphics::plot(value~Lab, data=data, main="", ylab=paste0(unique(data[,"analyte"])[1], " [",unique(data[,"unit"])[1],"]"))
  graphics::text(x=jitter(as.numeric(data[,"Lab"])), y=data[,"value"], labels = data[,"ID"], col=4)
}
