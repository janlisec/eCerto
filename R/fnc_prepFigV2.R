...
prepFigV2 <- function(tab = NULL, a = NULL, alpha = 0.05, k = 3, flt_outliers = flt_outliers) {
  ...
  graphics::mtext(text = expression(y==b[0]+b[1]*x), side = 3, line = -1.2, at = 0, adj = 0, cex = cex)
  graphics::mtext(text = bquote(b[0]==.(round(vals[,"b0"], 4))), side = 3, line = -2.4, at = 0, adj = 0, cex = cex)
  graphics::mtext(text = bquote(b[1]==.(round(vals[,"b1"], 4))), side = 3, line = -3.6, at = 0, adj = 0, cex = cex)
  graphics::mtext(text = paste("r =", round(stats::cor((stats::lm(Area_norm ~ Conc, data=df))$fitted.values, df$Area_norm), 4)), side = 3, line = -4.8, at = 0, adj = 0, cex = cex)
  graphics::par(mar=c(3,4,0,2)+0.1)
  tmp.x <- graphics::barplot(unname(e), ylim = res_rng, ann=F)
  flt <- e>=0
  graphics::text(x = tmp.x[flt], y = 0, labels = names(e)[flt], srt=90, adj=1.1, cex = 2/3)
  graphics::text(x = tmp.x[!flt], y = 0, labels = names(e)[!flt], srt=90, adj=-0.1, cex = 2/3)
  ...
  graphics::points(df, pch=4, col=2, cex=2)
  graphics::mtext(text = expression(y==b[0]+b[1]*x+b[2]*x^2), side = 3, line = -1.2, at = 0, adj = 0, cex = cex)
  graphics::mtext(text = bquote(b[0]==.(round(stats::coef(qm)[1], 4))), side = 3, line = -2.4, at = 0, adj = 0, cex = cex)
  graphics::mtext(text = bquote(b[1]==.(round(stats::coef(qm)[2], 4))), side = 3, line = -3.6, at = 0, adj = 0, cex = cex)
  graphics::mtext(text = bquote(b[2]==.(round(stats::coef(qm)[3], 4))), side = 3, line = -4.8, at = 0, adj = 0, cex = cex)
  graphics::mtext(text = paste("r =", round(stats::cor(qm$fitted.values, dfq$Area_norm), 4)), side = 3, line = -6, at = 0, adj = 0, cex = cex)
  graphics::par(mar=c(3,4,0,2)+0.1)
