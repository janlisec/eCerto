...
#' @description \code{fnc_check_RData} will check a file path if it specifies an
...
#' @keywords internal
#' @noRd
fnc_check_RData <- function(x = NULL) {
  ...
  shiny::validate(
    ...
    shiny::need(tolower(file.type) == "rdata","File extension different from RData. Please select a single RData file containing an object 'res'.")
  )
  file.type <- "RData"
  load_envir <- new.env()
  tryCatch({
